...
const initConnections: Connection[] = [
  {
    ...
    to: 0,
    type: "implementation",
    path: <Arrow from={1} to={0} type="implementation" key={0} />,
  },
  {
    from: 2,
    to: 1,
    type: "extension",
    path: <Arrow from={2} to={1} type="extension" key={1} />,
  },
  {
    from: 4,
    to: 1,
    type: "extension",
    path: <Arrow from={4} to={1} type="extension" key={2} />,
  },
];
...
export default function App() {
  ...
  const [classifiers, setClassifiers] =
    useState<ClassifierData[]>(initClassifiers);
  ...
  const updateClassifier = useCallback(
    (id: number, updates: Partial<ClassifierData>) => {
      setClassifiers((prev) => {
  ...
  const updateArrows = () => {
    for (const connection of connections) {
      ...
      const rect = document.querySelector(
        `rect[data-key="${datakey}"]`,
      ) as SVGRectElement;
      const textpath = document.querySelector(
        `textpath[data-key="${datakey}"]`,
      ...
      const text = document.querySelector(
        `text[data-key="text-${datakey}"]`,
      ) as SVGTextElement;
      const textbox = text.getBBox(); // Get the LOCAL coordinates
      if (p[0].x > p[3].x) {
        text.setAttribute(
          "transform",
          `rotate(180, ${textbox.x + textbox.width / 2}, ${textbox.y + textbox.height / 2})`,
        );
      } else {
  ...
  const onMouseMove = (e: MouseEvent<HTMLDivElement>) => {
    ...
    if (lastposition.current) {
      ...
      offset.current = {
        x: offset.current.x + dx / scale,
        y: offset.current.y + dy / scale,
      };
  ...
  const onKeyDown = (e: KeyboardEvent<HTMLDivElement>) => {
    ...
    onKeysPressed(e, "alt a", () =>
      setClassifiers((prev) => [
        ...prev,
        { id: prev.length, type: "abstract class", position: center },
      ]),
    );
    onKeysPressed(e, "alt c", () =>
      setClassifiers((prev) => [
        ...prev,
        { id: prev.length, type: "class", position: center },
      ]),
    );
    onKeysPressed(e, "alt i", () =>
      setClassifiers((prev) => [
        ...prev,
        { id: prev.length, type: "interface", position: center },
      ]),
    );
    onKeysPressed(e, "alt e", () =>
      setClassifiers((prev) => [
        ...prev,
        { id: prev.length, type: "enumeration", position: center },
      ]),
    );
  };
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      tabIndex={0}
      className="absolute w-screen h-screen outline-none "
    ...
    >
      <svg className="absolute w-screen h-screen pointer-events-none">
        <defs>
          ...
          >
            {/* Directed Association (filled arrowhead) */}{" "}
          ...
          <marker
            ...
            markerUnits="strokeWidth"
          >
            {/* Inheritance / Realization (hollow arrowhead) */}{" "}
            <path
          ...
          >
            ...
            <path d="M0,6 L6,0 L12,6 L6,12 Z" fill="white" />
          </marker>
